...
// Allow either ADMIN_API_TOKEN via GET (quick) or admin session
$admin_token = $_GET['admin_token'] ?? null;
$expected = getenv('ADMIN_API_TOKEN') ?: '';
$session_access = false;
if ($admin_token && $expected && hash_equals($expected, $admin_token)) {
    // token-based access (allowed via GET)
} else {
