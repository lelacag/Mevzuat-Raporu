...
try {
    $stmt = $db->prepare("UPDATE user_custom_badges SET is_approved = 1, approved_at = NOW() WHERE id = ?");
    $stmt->execute([$badge_id]);
    log_admin_action('approve_badge', 'approved badge_id=' . $badge_id, get_current_user_id());
    $_SESSION['flash'] = 'Badge approved';
    header('Location: ' . BASE_PATH . '/admin/premium_badges.php');
    exit;
} catch (Exception $e) {
