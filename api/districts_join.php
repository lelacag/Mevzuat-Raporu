...
try {
    ...
    if (!$district) {
        ...
        echo http_build_query(['success' => '0', 'error' => 'District not found']);
        exit;
    }
    ...
    if ($check->fetch()) {
        echo http_build_query(['success' => '0', 'error' => 'Already a member of this district']);
        exit;
    }
    ...
    if ($current >= $district['max_members']) {
        http_response_code(400);
        echo http_build_query(['success' => '0', 'error' => 'District has reached maximum member capacity']);
        exit;
    }
