...
$count_stmt = query("SELECT COUNT(*) as c FROM users WHERE deleted_at IS NULL AND is_online = 1");
...
$stmt = query("SELECT id, username, bio, is_premium, last_activity FROM users WHERE deleted_at IS NULL AND is_online = 1 ORDER BY last_activity DESC LIMIT ? OFFSET ?", [$per_page, $offset]);
