...
try {
    $open_reports_stmt = query("SELECT COUNT(*) as count FROM reports WHERE status = 'open'");
    $open_reports_count = (int)($open_reports_stmt->fetch()['count'] ?? 0);
} catch (Exception $e) {
...
<?php endif; ?>
    ...
    <a href="<?= BASE_PATH ?>/admin/reports.php" class="<?= $active === 'reports' ? 'active' : '' ?>">
        Raporlar
        <?php if ($open_reports_count > 0): ?>
            <span class="notification-badge"><?= $open_reports_count ?></span>
        <?php endif; ?>
    </a>
<?php endif; ?>
...
<?php endif; ?>
    ...
    <a href="<?= BASE_PATH ?>/admin/premium_users.php" class="<?= ($active === 'premium' && strpos($_SERVER['REQUEST_URI'], 'premium_settings.php') === false) ? 'active' : '' ?>">Premium Yönetimi</a>
    <?php /* IAP Status nav entry removed; module inactive. Restore when IAP_ENABLED=true. */ ?>
...
<?php endif; ?>
    <?php if (is_admin() || admin_has_perm(null, 'manage_events')): ?>
...
<?php endif; ?>
    ...
    <a href="<?= BASE_PATH ?>/admin/announcements.php" class="<?= strpos($_SERVER['REQUEST_URI'], 'announcements.php') !== false && !isset($_GET['action']) ? 'active' : '' ?>">📢 Duyurular</a>
    <a href="<?= BASE_PATH ?>/admin/announcements.php?action=celebrations" class="<?= strpos($_SERVER['REQUEST_URI'], 'announcements.php') !== false && strpos($_SERVER['REQUEST_URI'], 'action=celebrations') !== false ? 'active' : '' ?>">🎉 Kutlamalar</a>
    <a href="<?= BASE_PATH ?>/admin/announcements.php?action=bulk_optin" class="<?= strpos($_SERVER['REQUEST_URI'], 'announcements.php') !== false && isset($_GET['action']) && $_GET['action'] === 'bulk_optin' ? 'active' : '' ?>">✉️ Bulk Mail</a>
<?php endif; ?>
    <?php if (is_admin()): ?>
