...
$metadata = [];
$payload_raw = base64_decode($sub['vendor_payload'] ?? '', true);
if ($payload_raw !== false) {
    $decoded = unserialize($payload_raw, ['allowed_classes' => false]);
    if (is_array($decoded)) {
        $metadata = $decoded;
    }
}
...
try {
    if ($platform === 'android') {
        ...
        if (!$res['success']) {
            $_SESSION['flash_error'] = 'Google Play verification failed: ' . print_r($res, true);
            header('Location: ' . BASE_PATH . '/admin/premium_users.php#reconcile');
            exit;
        }
        ...
        $db->prepare("UPDATE premium_subscriptions SET vendor_transaction_id = ?, vendor_status = ?, vendor_payload = ?, validated_at = NOW() WHERE id = ?")
            ->execute([$vendor_tx_id, $vendor_status, base64_encode(serialize($payload)), $id]);
