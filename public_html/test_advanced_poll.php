...
// Clear stale published test draft if its DB record no longer exists or it no longer has an active linked post.
if (!empty($_SESSION['test_anket_final']['id'])) {
    $final_test_id = (int)$_SESSION['test_anket_final']['id'];
    $fresh_test = get_test_by_id($final_test_id);
    if (!$fresh_test) {
        unset($_SESSION['test_anket_final']);
    } else {
        $stmt = db_connect()->prepare(
            "SELECT 1 FROM post_tests pt
             LEFT JOIN posts p ON pt.post_id = p.id AND p.deleted_at IS NULL
             LEFT JOIN group_posts gp ON pt.group_post_id = gp.id
             WHERE pt.test_id = ? AND (p.id IS NOT NULL OR gp.id IS NOT NULL)
             LIMIT 1"
        );
        $stmt->execute([$final_test_id]);
        if (!$stmt->fetchColumn()) {
            unset($_SESSION['test_anket_final']);
        } else {
            if (!empty($fresh_test['thresholds']) && is_array($fresh_test['thresholds'])) {
                foreach ($fresh_test['thresholds'] as &$th) {
                    if (!isset($th['out']) && isset($th['out_text'])) {
                        $th['out'] = $th['out_text'];
                    }
                }
                unset($th);
            }
            $_SESSION['test_anket_final'] = $fresh_test;
        }
    }
}
    ...
    <aside class="sidebar sidebar-left">
        ...
        <div class="sidebar-help" style="margin-top:12px;">
            ...
            <ul class="sidebar-help-list">
                <li>Eklemeleri sunucuda "➕ Ekle" ile yapıyoruz.</li>
    ...
    <main class="content-area narrow">
        ...
        <?php if ($rookie_restricted): ?>
            <div class="card-box padded admin-note-error">
                ...
                <p class="muted">Hesabınız <strong>rookie</strong> olduğu için Tahlil (çok sorulu test) oluşturma özelliği devre dışı. <a href="<?= BASE_PATH ?>/premium">Premium</a> olarak erişim kazanabilirsiniz.</p>
            </div>
        <?php else: ?>
        <form method="POST" class="card-box padded">
            ...
            <!-- Form header -->
            <div class="tahlil-form-header">
                <span class="tahlil-form-icon">📋</span>
                <p class="tahlil-form-title"><?= !empty($editing_id) ? 'Tahlili Düzenle' : 'Yeni Tahlil Oluştur' ?></p>
            </div>

            <!-- Adım 1: Başlık -->
            <div class="tahlil-step">
                <p class="tahlil-step-label">Adım 1 — Başlık</p>
                <label for="tahlil-title" style="font-size:13px;font-weight:600;color:#333;display:block;margin-bottom:6px;">Tahlil Başlığı <span style="color:#c0392b">*</span></label>
                <input id="tahlil-title" type="text" name="title" value="<?= htmlspecialchars($state['title']) ?>" placeholder="Örn: Liderlik Tarzın Nedir?" style="width:100%;box-sizing:border-box;">
                <p class="tahlil-step-hint">Başlık paylaşıldığında ve arama sonuçlarında görünür.</p>
            </div>

            <!-- Adım 2: Sorular -->
            <div class="tahlil-step">
                <p class="tahlil-step-label">Adım 2 — Sorular &amp; Seçenekler</p>
                <?php foreach ($state['questions'] as $qi => $q): $qidx = $qi+1; ?>
                    <div class="question-block">
                        <div class="question-header">
                            <span class="question-num-badge"><?= $qidx ?></span>
                            <input class="question-text" type="text" name="question_text_<?= $qidx ?>" value="<?= htmlspecialchars($q['question_text']) ?>" placeholder="Soru metnini yazın…">
                        </div>
                        <div class="question-options">
                        <?php foreach ($q['options'] as $oi => $opt): $oidx = $oi+1; ?>
                            <div class="opt">
                                <span class="opt-seq"><?= chr(64 + $oidx) ?></span>
                                <input class="opt-points" type="text" name="opt_points_<?= $qidx ?>_<?= $oidx ?>" value="<?= htmlspecialchars($opt['points']) ?>" placeholder="Puan" title="Puan">
                                <input class="opt-label" type="text" name="opt_label_<?= $qidx ?>_<?= $oidx ?>" value="<?= htmlspecialchars($opt['label']) ?>" placeholder="Seçenek metni">
                            </div>
                        <?php endforeach; ?>
                        </div>
                        <div class="question-actions">
                            <button type="submit" name="add_option" value="<?= $qidx ?>" class="btn-small">+ Seçenek Ekle</button>
                            <button type="submit" name="delete_question" value="<?= $qidx ?>" class="btn-small btn-danger">Soruyu Sil</button>
                        </div>
                        <?php if (isset($pending_delete) && $pending_delete === $qi): ?>
                            <div class="form-alert form-alert-warning" style="margin-top:10px;">
                                <p style="margin:0 0 8px 0;"><strong>Bu soruyu silmek istediğinize emin misiniz?</strong></p>
                                <button type="submit" name="confirm_delete_question" value="<?= $qidx ?>" class="btn-small btn-danger">Evet, Sil</button>
            ...
            <!-- Adım 3: Eşikler -->
            <div class="tahlil-step">
                <p class="tahlil-step-label">Adım 3 — Sonuç Eşikleri</p>
                <p class="tahlil-step-hint" style="margin-bottom:10px;">Toplam puan ≤ değer ise ilgili sonuç gösterilir. Eşikleri <strong>artan</strong> sırada girin.</p>
                <?php foreach ($state['thresholds'] as $i => $t): $idx = $i+1; ?>
                    <div class="tahlil-threshold-row">
                        <span class="tahlil-threshold-sym">≤</span>
                        <input class="tahlil-th-value" type="text" name="th_value_<?= $idx ?>" value="<?= htmlspecialchars($t['value']) ?>" placeholder="Puan">
                        <span class="tahlil-threshold-arrow">→</span>
                        <input class="tahlil-th-out" type="text" name="th_out_<?= $idx ?>" value="<?= htmlspecialchars($t['out']) ?>" placeholder="Sonuç metni">
                    </div>
                <?php endforeach; ?>
                <div style="margin-top:4px;">
                    <button type="submit" name="add_threshold" class="btn-small">+ Eşik Ekle</button>
                </div>
            </div>

            <!-- Submit zone -->
            <div class="tahlil-submit-zone">
                <?php if (!empty($state['group_slug'])): ?>
                    <input type="hidden" name="group_slug" value="<?= htmlspecialchars($state['group_slug']) ?>">
                    <span class="tahlil-group-note">Grup olarak paylaşılacak: <em><?= htmlspecialchars($state['group_slug']) ?></em></span>
                <?php endif; ?>
                <?php if (!empty($editing_id)): ?>
                    <input type="hidden" name="edit_test_id" value="<?= (int)$editing_id ?>">
                    <?php $cancel_url = BASE_PATH . '/tahlil/duzenle/' . (int)$editing_id; ?>
        ...
        <?php if (isset($_SESSION['test_anket_final'])): $final = $_SESSION['test_anket_final']; ?>
            <div class="tahlil-preview-panel">
                <h3>🔍 Tahlili Dene</h3>
                <p style="font-size:14px;margin-bottom:14px;color:#333;"><strong><?= htmlspecialchars($final['title'] ?? '') ?></strong></p>
                <form method="POST">
                    <?php foreach (($final['questions'] ?? []) as $qi => $q): ?>
                        <div class="tahlil-preview-q">
                            <div class="tahlil-preview-q-text"><?= $qi+1 ?>. <?= htmlspecialchars($q['question_text'] ?? '') ?></div>
                            <?php foreach (($q['options'] ?? []) as $oi => $opt): ?>
                                <?php $val = isset($opt['id']) ? $opt['id'] : $oi; ?>
                                <div class="tahlil-preview-opt">
                ...
                <?php if (!empty($_SESSION['test_result'])): $r = $_SESSION['test_result']; ?>
                    <div style="margin-top:14px;" class="form-alert form-alert-success">Toplam Puan: <strong><?= intval($r['sum']) ?></strong> — <?= htmlspecialchars($r['out']) ?></div>
