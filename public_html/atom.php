...
try {
    ...
    $atomLastModified = $lmRow && $lmRow['lm'] ? gmdate('D, d M Y H:i:s', strtotime($lmRow['lm'])) . ' GMT' : gmdate('D, d M Y H:i:s') . ' GMT';
    $atomEtag = '"' . md5(($lmRow['lm'] ?? '') . '|' . ($lmRow['c'] ?? '0')) . '"';
} catch (Exception $e) {
    $updated = date('c');
    $atomLastModified = gmdate('D, d M Y H:i:s') . ' GMT';
    $atomEtag = '"' . md5($atomLastModified) . '"';
}

$baseUrl = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 'https' : 'http') . '://' . ($_SERVER['HTTP_HOST'] ?? 'localhost') . rtrim(BASE_PATH, '/');
header('Content-Type: application/atom+xml; charset=utf-8');
header('Cache-Control: public, max-age=60');
header('Last-Modified: ' . $atomLastModified);
header('ETag: ' . $atomEtag);

// Honour conditional GET requests (RFC 7232)
if (isset($_SERVER['HTTP_IF_NONE_MATCH']) && $_SERVER['HTTP_IF_NONE_MATCH'] === $atomEtag) {
    http_response_code(304);
    exit;
}
if (!isset($_SERVER['HTTP_IF_NONE_MATCH']) && isset($_SERVER['HTTP_IF_MODIFIED_SINCE'])
    && strtotime($_SERVER['HTTP_IF_MODIFIED_SINCE']) >= strtotime($atomLastModified)) {
    http_response_code(304);
    exit;
}
