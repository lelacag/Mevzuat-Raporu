...
if ($login_result === true) {
    ...
    if ($force_url_session || empty($_COOKIE) || !isset($_COOKIE[$session_cookie_name])) {
        ...
        if (!$user_id) {
            ...
            if ($sid) {
                ...
                $url = $scheme . '://' . $base_host . BASE_PATH . '/?sid=' . rawurlencode($sid);
                ...
                $action = $scheme . '://' . $base_host . BASE_PATH . '/';
                ...
                $_SESSION['login_error'] = 'Oturum başlatılamadı. Lütfen tarayıcı ayarlarınızı kontrol edin ve tekrar deneyin.';
                header('Location: ' . BASE_PATH . '/landing.php');
                exit;
            }
        }
    ...
    if ($error === 'account_deleted') {
        ...
        error_log('Login failed (other error) for ' . $username . ' result: ' . print_r($login_result, true));
    }
} else {
    // Login failed
