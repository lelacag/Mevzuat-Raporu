...
try {
    ...
    if (isset($data['posts']) && is_array($data['posts'])) {
        foreach ($data['posts'] as $post) {
            try {
                ...
                if (!$post_id) {
                    $findStmt = $pdo->prepare("SELECT id FROM district_posts WHERE post_uuid = ?");
                    $findStmt->execute([$post['uuid']]);
                    $post_id = $findStmt->fetchColumn();
                }
        ...
        foreach ($data['actions'] as $action) {
            try {
                ...
                $stmt->execute([
                    ...
                    $user_id,
                    $action['type'],
                    base64_encode(serialize($action['data'])),
    ...
    echo http_build_query([
        'success' => '1',
        'synced_posts' => count($synced_posts),
        'synced_actions' => count($synced_actions),
    ...
    echo http_build_query(['success' => '0', 'error' => 'Sync failed: ' . $e->getMessage()]);
}
