...
if (!$parent) {
    ...
    header('Location: ' . event_view_url($event_id, $parent['title'] ?? ''));
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'create_reply') {
    ...
    try {
        ...
        header('Location: ' . event_view_url($event_id, $parent['title'] ?? '') . '#comment-' . $newId);
        exit;
    } catch (Exception $e) {
        ...
        <article class="card-box padded">
            ...
            <form method="POST" class="post-form">
                ...
                <div class="post-form-actions">
                    <a href="<?= htmlspecialchars(event_view_url($event_id, $parent['title'] ?? '')) ?>" class="btn btn-cancel">İptal</a>
