...
$has_exported_data = !empty($_SESSION['user_data_exported_at']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['csrf_token']) || !verify_csrf_token($_POST['csrf_token'])) {
        ...
        if (empty($_SESSION['user_data_exported_at'])) {
            $errors[] = 'Lütfen hesabınıza ait verilerinizi indirip saklayın.';
        }
        if (empty($_POST['download_acknowledged'])) {
            $errors[] = 'Lütfen verilerinizi indirdiğinizi onaylayın.';
        }
    ...
    <main class="content-area padded">
        ...
        <?php if ($success): ?>
            ...
            <div class="text-center padded">
                ...
                <a href="<?= home_url() ?>" class="btn btn-success mt-12">Ana Sayfaya Dön</a>
            </div>
        ...
        <?php elseif ($step === 1): ?>
            ...
            <div class="text-center">
                ...
                <div class="alert alert-info">
                    <p style="margin: 0 0 10px 0;">Hesabınızı silmeden önce verilerinizi indirdiğinizi onaylamanız gerekiyor.</p>
                    <a href="<?= BASE_PATH ?>/download_user_data.php" class="btn btn-primary mt-10">Verilerimi İndir</a>
                    <?php if ($has_exported_data): ?>
                ...
                <form method="POST" class="mt-12">
                    ...
                    <div class="checkbox-row" style="margin-bottom: 18px; display: flex; align-items: center; justify-content: center; gap: 12px;">
                        <label style="display: flex; align-items: center; gap: 10px;">
                            <input type="checkbox" name="download_acknowledged" value="1" <?= $has_exported_data ? 'checked' : '' ?>>
                            <span>Verilerimi indirdim ve sakladım.</span>
                        </label>
                    </div>

                    <div class="flex-row justify-center">
                        <button type="submit" class="btn btn-danger btn-lg">Evet, Hesabımı Sil</button>
                        <a href="<?= BASE_PATH ?>/profil/duzenle" class="btn btn-secondary">İptal</a>
                    </div>
                </form>
            </div>
