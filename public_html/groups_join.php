...
$redirect = $_POST['redirect'] ?? BASE_PATH . '/topluluklar';
...
try {
    ...
    if (!empty($group['is_private'])) {
        ...
        if ($group['entry_question'] && $answer === '') {
            ...
            header('Location: ' . BASE_PATH . '/g/' . urlencode($group['slug']));
            exit;
        }
        try {
            ...
            if (!$exists->fetch()) {
                ...
                try {
                    ...
                    $stmtNReq = $pdo->prepare("INSERT INTO notifications (user_id, type, text, from_user_id, group_id, target_url, created_at) VALUES (?, 'system', ?, ?, ?, ?, NOW())");
                    $stmtNReq->execute([$user_id, $textReq, $user_id, $group_id, group_url($group['slug'])]);
                ...
                try {
                    ...
                    if (!empty($admins)) {
                        ...
                        foreach ($admins as $admin_id) {
                            ...
                            $stmtN->execute([$admin_id, $text, $user_id, $group_id, group_edit_url($group['slug']) . '?tab=applications']);
                        }
                    }
                    ...
                    if (!empty($group['created_by']) && (empty($admins) || !in_array($group['created_by'], $admins))) {
                        ...
                        $stmtNO = $pdo->prepare("INSERT INTO notifications (user_id, type, text, from_user_id, group_id, target_url, created_at) VALUES (?, 'system', ?, ?, ?, ?, NOW())");
                        $stmtNO->execute([$group['created_by'], $textOwner, $user_id, $group_id, group_edit_url($group['slug']) . '?tab=applications']);
                    }
