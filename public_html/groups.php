...
// Get all groups with member counts and recent activity
$groups = $pdo->query("
    SELECT 
        g.*,
        ...
        MAX(gp.created_at) as last_posted_at
    FROM groups_table g
    LEFT JOIN users u ON g.created_by = u.id
    LEFT JOIN group_members gm ON g.id = gm.group_id
    LEFT JOIN group_posts gp ON g.id = gp.group_id
    GROUP BY g.id
    ORDER BY last_posted_at DESC, member_count DESC, g.name ASC
")->fetchAll();

// Get user's groups if logged in
$user_groups = [];
$user_group_joined_at = [];
$my_groups = [];
if ($user_id) {
    $stmt = $pdo->prepare("
        SELECT group_id 
    ...
    $user_groups = array_column($stmt->fetchAll(PDO::FETCH_ASSOC), 'group_id');

    $stmtJoined = $pdo->prepare("SELECT group_id, joined_at FROM group_members WHERE user_id = ?");
    $stmtJoined->execute([$user_id]);
    foreach ($stmtJoined->fetchAll(PDO::FETCH_ASSOC) as $row) {
        $user_group_joined_at[$row['group_id']] = $row['joined_at'];
    }
    ...
    if (!empty($user_groups)) {
        ...
        $stmtMy = $pdo->prepare("
            ...
            GROUP BY g.id
            ORDER BY last_posted_at DESC, g.name ASC
        ");
...
<div class="main-container groups-layout">
    ...
    <main class="content-area narrow">
        ...
        <div class="groups-grid padded">
            ...
            <?php
                $group_joined_at = $user_group_joined_at[$group['id']] ?? null;
                $group_viewed_at = get_group_viewed_at($user_id, $group['id']);
                $group_is_new = false;
                if (!empty($user_id) && !empty($group['last_posted_at'])) {
                    $posted_ts = strtotime($group['last_posted_at']);
                    if (!empty($group_viewed_at) && $posted_ts > strtotime($group_viewed_at)) {
                        $group_is_new = true;
                    } elseif (empty($group_viewed_at) && !empty($group_joined_at) && $posted_ts > strtotime($group_joined_at)) {
                        $group_is_new = true;
                    }
                }
            ?>
            <article class="group-card<?= $group_is_new ? ' group-card-new' : '' ?>">
                <!-- Header with name and button -->
                <div class="group-card-header">
                    <div class="group-card-title">
                        <a href="<?= group_url($group['slug']) ?>" class="group-link"><?= htmlspecialchars($group['name']) ?></a>
                        <?php if ($group_is_new): ?>
                            <span class="badge badge-primary" style="margin-left:10px;">Yeni</span>
                        <?php endif; ?>
                    </div>
