...
$render_tab = ($tab === 'users') ? 'kullanici' : $tab;
...
if ($view === 'user_posts') {
    ...
    $stmt = query(
        "SELECT id, username, created_at FROM users WHERE deleted_at IS NULL AND email_verified = 1 ORDER BY created_at DESC LIMIT 50"
    );
    ...
    $count_stmt = $pdo->query("
        ...
        WHERE u.is_online = 1 AND u.email_verified = 1 AND p.created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY)
    ");
    $total_count = $count_stmt->fetch()['total'];
    $total_pages = ceil($total_count / $items_per_page);
...
} elseif ($tab === 'users' && $search_query) {
    // User search
    $search_title = 'Kullanıcılar: "' . htmlspecialchars($search_query) . '"';
    $stmt = query(
    ...
    // Groups search
    $search_title = 'Topluluklar: "' . htmlspecialchars($query) . '"';
    $stmt = query(
        "SELECT 
    ...
    <aside class="sidebar sidebar-left">
        <div class="sidebar-section">
            ...
            <ul class="sidebar-list">
                ...
                <li><a href="<?= search_url() ?>"><span class="menu-icon icon-search" aria-hidden="true"></span>Arama</a></li>
            </ul>
        </div>
    ...
    <main class="content-area">
        <h1 class="section-title">🔍 Arama</h1>
          ...
          <div class="tabs">
            ...
            ?>
                ...
                <a href="<?= $base ?>?tab=kullanici<?= $qparam ?>" 
        ...
        <div class="post-form-container mb-20">
            <form method="GET" action="<?= search_url() ?>" class="form-padded">
                <input type="hidden" name="tab" value="<?= htmlspecialchars($render_tab) ?>">
                <input 
                    type="text" 
                    name="q" 
