...
function init_captcha() {
    ...
    $ip = get_client_ip();
    if ($ip !== '0.0.0.0' && is_captcha_generation_rate_exceeded($ip, CAPTCHA_GENERATION_LIMIT, CAPTCHA_GENERATION_WINDOW)) {
...
function verify_captcha($user_input, $token) {
    ...
    if (time() - $data['time'] < CAPTCHA_MIN_SECONDS) {
        ...
        $ip = get_client_ip();
        if ($ip !== '0.0.0.0') record_captcha_ip_failure($ip);
...
// Save captcha data to DB store (token -> serialized payload)
...
function load_captcha_from_store($token) {
    try {
        ...
        $payload_raw = base64_decode($row['payload'], true);
        if ($payload_raw === false) {
            return false;
        }
        $payload = unserialize($payload_raw, ['allowed_classes' => false]);
