...
try {
    $lmStmt = $pdo->prepare("SELECT MAX(created_at) AS lm, COUNT(*) AS c FROM group_posts WHERE group_id = ?");
...
if (isset($_SERVER['HTTP_IF_NONE_MATCH']) && $_SERVER['HTTP_IF_NONE_MATCH'] === $etagGroup) {
    http_response_code(304);
    exit;
}
if (!isset($_SERVER['HTTP_IF_NONE_MATCH']) && isset($_SERVER['HTTP_IF_MODIFIED_SINCE'])
    && strtotime($_SERVER['HTTP_IF_MODIFIED_SINCE']) >= strtotime($lastModifiedGroup)) {
    http_response_code(304);
    exit;
}
...
<?php
try {
    $stmt = $pdo->prepare("SELECT gp.id, gp.content, gp.created_at, u.username FROM group_posts gp JOIN users u ON gp.user_id = u.id WHERE gp.group_id = ? ORDER BY gp.created_at DESC LIMIT 50");
