    ...
    <main class="content-area">
        ...
        <div class="post-form-container">
            ...
            <form method="POST" action="<?= BASE_PATH ?>/api/group_post_edit.php" class="post-form">
                ...
                <?php
                    $edit_post_limit = function_exists('get_user_post_limit') ? get_user_post_limit($user_id) : MAX_POST_LENGTH;
                    $edit_textarea_attrs = $edit_post_limit > 0 ? ' maxlength="' . (int)$edit_post_limit . '"' : '';
                ?>
                <textarea name="content" id="content" rows="8" required placeholder="Gönderinizi buraya yazın..." class="input-full textarea-large"<?= $edit_textarea_attrs ?>><?= htmlspecialchars($post['content']) ?></textarea>
                
                <div class="post-form-actions">
                    <span class="char-count">
                        <?php if ($edit_post_limit == 0): ?>
                            <?= mb_strlen($post['content']) ?> karakter · Sınırsız (Premium)
                        <?php else: ?>
                            <?= mb_strlen($post['content']) ?> / <?= (int)$edit_post_limit ?> karakter
                        <?php endif; ?>
                    </span>
                    <div class="flex-row">
        ...
        <div class="sidebar-section">
            ...
            <div style="padding:10px;font-size:12px;color:#666;line-height:1.6;">
                ...
                <p style="margin:5px 0;">
                    <a href="<?= BASE_PATH ?>/g/<?= urlencode($slug) ?>"><?= htmlspecialchars($post['group_name']) ?></a>
                </p>
            </div>
        </div>
    </aside>
