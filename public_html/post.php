...
if ($user_id && isset($_GET['action']) && $_GET['action'] === 'new') {
    ...
    <div class="main-container post-new">
        ...
        <aside class="sidebar sidebar-left">
            <a href="<?= home_url() ?>" class="btn btn-primary">← Geri Dön</a>
        </aside>
...
// so downstream compare handling works for both '/latest', '/son-duzenleme', and '/gecmis'.
if (!isset($_GET['compare'])) {
    ...
    if (strpos($reqPath, '/karsilastirma/son-duzenleme') !== false) {
        ...
        $_GET['compare'] = 'latest';
    } elseif (strpos($reqPath, '/karsilastirma/gecmis') !== false || strpos($reqPath, '/karsilastirma/history') !== false) {
...
if ($user_id && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'create_reply') {
    ...
    if (empty($content)) {
        ...
        if (isset($res['error'])) {
            ...
            if ($res['approved']) {
                $_SESSION['flash'] = 'Yanıntınız paylaşıldı.';
            } else {
                $_SESSION['flash'] = 'Yanınıtınız onay bekliyor; onaylandıktan sonra görünür.';
            }
...
if ($compare) {
    ...
    if ($compare === 'history') {
        ...
        // Ensure timestamps for old schema records
        foreach ($edits as &$_e) {
            if (empty($_e['created_at']) && !empty($_e['edited_at'])) {
                $_e['created_at'] = $_e['edited_at'];
            }
        }
        unset($_e);
        require_once __DIR__ . '/includes/header.php';
            ...
            <main class="content-area narrow">
                <div class="card-box padded">
                    ...
                    <?php else: ?>
                        ...
                        ?>
                            <li>
                                <a href="<?= htmlspecialchars(get_post_compare_url($post['id'], $e['id'], $post['username'] ?? null), ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars(format_time($e['created_at']), ENT_QUOTES, 'UTF-8') ?></a>
                                — <?= $editor_label ?>
                            </li>
                        <?php endforeach; ?>
                        </ol>
                    <?php endif; ?>
                    <?php $compare_home = htmlspecialchars(get_post_compare_url($post['id'], 'latest', $post['username'] ?? null), ENT_QUOTES, 'UTF-8'); ?>
    ...
    if ($compare === 'latest' || $compare === 'son-duzenleme') {
        $stmt = db_connect()->prepare("SELECT * FROM post_edits WHERE post_id = ? ORDER BY COALESCE(created_at, edited_at) DESC LIMIT 1");
    ...
    <div class="main-container single-column">
        <main class="content-area narrow">
            <div class="card-box padded">
                ...
                <?php
                    $edit_ts = $edit_row['created_at'] ?? $edit_row['edited_at'] ?? '';
                    // Resolve before/after content — fall back to original_content + current post content for old schema records
                    $diff_old = $edit_row['previous_content'] ?? null;
                    $diff_new = $edit_row['new_content'] ?? null;
                    if ($diff_old === null && $diff_new === null && !empty($edit_row['original_content'])) {
                        $diff_old = $edit_row['original_content'];
                        $diff_new = $post['content'] ?? '';
                    }
                    $diff_old = (string)$diff_old;
                    $diff_new = (string)$diff_new;
                ?>
                <div class="muted small">Gönderi #: <?= (int)$post['id'] ?> · Düzenlendi: <?= htmlspecialchars($edit_ts) ?></div>
                <hr>
                <?php $history_link = htmlspecialchars(get_post_compare_url($post['id'], 'gecmis', $post['username'] ?? null), ENT_QUOTES, 'UTF-8'); ?>
                ...
                <div class="diff-columns">
                    <div class="diff-col diff-old">
                        <h3>Önce</h3>
                        <div class="diff-body"><?= render_diff_old_html($diff_old, $diff_new) ?></div>
                    </div>
                    <div class="diff-col diff-new">
                        <h3>Sonra</h3>
                        <div class="diff-body"><?= render_diff_new_html($diff_old, $diff_new) ?></div>
                    </div>
                </div>
...
if (use_clean_urls()) {
    ...
    $is_canonical = false;
    if ($req_username) {
        $normalized_req = mb_strtolower(trim($req_username), 'UTF-8');
        $post_username = mb_strtolower(trim($post['username'] ?? ''), 'UTF-8');
        $is_canonical = ($normalized_req === $post_username);
        ...
        if (!$is_canonical) {
            $post_slug = get_user_slug($post['username'] ?? '');
            if (!empty($post_slug) && $normalized_req === mb_strtolower(trim($post_slug), 'UTF-8')) {
                $is_canonical = true;
            }
        }
    }
    ...
    if (!$is_canonical) {
        ...
        $target_path = parse_url($loc, PHP_URL_PATH) ?: '';
...
$post['comment_count'] = (int)$total_replies;
    ...
    <aside class="sidebar sidebar-right">
        <div class="sidebar-section">
            ...
            <div class="sidebar-info">
                ...
                <p class="meta-row"><strong>Yanıt:</strong> <?= (int)($total_replies ?? count($replies)) ?></p>
