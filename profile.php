...
if ($current_user_id && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'follow') {
        ...
        // If neither publish nor schedule_submit, skip
...
// Count photos the user has uploaded
try {
    $pdo = $pdo ?? db_connect();
    $stmt = $pdo->prepare("SELECT COUNT(*) AS c FROM user_images WHERE user_id = ? AND deleted_at IS NULL");
    $stmt->execute([$profile_user_id]);
    $photos_count = (int)($stmt->fetch()['c'] ?? 0);
} catch (Exception $e) {
    error_log('profile photos count error: ' . $e->getMessage());
    $photos_count = 0;
}
...
try {
    $ecStmt = $pdo->prepare("SELECT c.*, u.username, u.is_premium, e.title AS event_title, (SELECT COUNT(*) FROM events_comments ec WHERE ec.parent_id = c.id AND ec.deleted_at IS NULL) AS replies_count FROM events_comments c JOIN users u ON c.user_id = u.id JOIN events e ON c.event_id = e.id WHERE c.user_id = ? AND c.deleted_at IS NULL ORDER BY c.created_at DESC LIMIT 20");
...
// Only show tier badge when user is fully approved; unapproved rookies show only 'Yeni Gelen'
$best_tier_badge = (!empty($profile_user['is_approved']) && function_exists('get_user_best_badge')) ? get_user_best_badge($profile_user_id) : null;
$_profile_is_rookie = empty($profile_user['is_approved']);
?>
    ...
    <aside class="sidebar sidebar-left sidebar-card">
        <div class="sidebar-section mb-12">
            ...
            <?php
                ...
                $notifications_link = notification_url();
                $groups_link = BASE_PATH . (use_clean_urls() ? '/topluluklar' : '/groups.php');
                $events_link = events_url();
                $favorites_link = favorites_url();
                $profile_link = null;
                if (!empty($current_user) && !empty($current_user['username'])) {
                    $profile_link = user_url($current_user['username']);
                } elseif (!empty($profile_user['username'])) {
                    $profile_link = user_url($profile_user['username']);
                }
                $settings_link = settings_url();
                $unread_notif_count = $current_user_id ? get_unread_count($current_user_id) : 0;
            ?>
            <ul class="side-menu no-margin">
                <li><a href="<?= $home_link ?>"><span class="menu-icon icon-home" aria-hidden="true"></span>Ana Sayfa</a></li>
                <li><a href="<?= $search_link ?>"><span class="menu-icon icon-search" aria-hidden="true"></span>Ara</a></li>
                <li>
                    <a href="<?= $notifications_link ?>"><span class="menu-icon icon-bell" aria-hidden="true"></span>Bildirimler</a>
                    <?php if ($unread_notif_count > 0): ?>
                        <span class="badge red"><?= $unread_notif_count ?></span>
                    <?php endif; ?>
                </li>
                <li><a href="<?= $groups_link ?>"><span class="menu-icon icon-users" aria-hidden="true"></span>Topluluklar</a></li>
                <li><a href="<?= $events_link ?>"><span class="menu-icon icon-calendar" aria-hidden="true"></span>Etkinlikler</a></li>
                <li><a href="<?= $favorites_link ?>"><span class="menu-icon icon-star" aria-hidden="true"></span>Favoriler</a></li>
                <?php if ($profile_link): ?>
                    <li><a href="<?= $profile_link ?>"><span class="menu-icon icon-user" aria-hidden="true"></span>Profil</a></li>
                <?php endif; ?>
                <li><a href="<?= $settings_link ?>"><span class="menu-icon icon-settings" aria-hidden="true"></span>Ayarlar</a></li>
            </ul>
        </div>
        ...
        <section class="profile-header">
            ...
            <div class="profile-info">
                <div class="profile-username-wrap">
                    ...
                    <!-- ROW 1: username + inline meta (left) | edit/follow/admin (right) -->
                    <div class="profile-identity-row">
                        ...
                        <div class="profile-action-buttons">
                            <?php if ($is_own_profile): ?>
                                <a href="<?= BASE_PATH ?>/profil/duzenle" class="edit-profile-btn"><?= t('profile_edit_btn') ?></a>
                            <?php endif; ?>
                    ...
                    <!-- ROW 2: badges only (custom premium + tier) -->
                    <?php
                        $custom_badge = is_user_premium($profile_user_id) ? get_user_custom_badge($profile_user_id) : null;
                        $color_map = [
                    ...
                    ?>
                    <?php if ($custom_badge || $best_tier_badge || $_profile_is_rookie): ?>
                    <div class="profile-badges-row">
                        <?php if ($_profile_is_rookie): ?>
                            <span class="badge badge-rookie profile-badge">Yeni Gelen</span>
                        <?php endif; ?>
                        <?php if ($best_tier_badge): ?>
                            <span class="badge badge-tier profile-badge"><?= htmlspecialchars($best_tier_badge['name']) ?></span>
                        <?php endif; ?>
                        <?php if ($custom_badge): ?>
                            <span class="custom-badge custom-badge-<?= $color_class ?>"><?= htmlspecialchars($custom_badge['badge_text']) ?></span>
                        <?php endif; ?>
                    </div>
                    <?php endif; ?>
                ...
                <div class="profile-stats">
                    ...
                    <?php if ($photos_count > 0 || $is_own_profile): ?>
                    <a href="<?= BASE_PATH ?>/fotograf/<?= rawurlencode($profile_user['username']) ?>" class="stat">
                        <span class="stat-value"><?= $photos_count ?></span>
            ...
            <div class="profile-actions">
                <?php if ($is_own_profile): ?>
                    ...
                    <div class="post-form-container">
                        ...
                        <?php else: ?>
                            <div class="tag-navigation tag-navigation-empty" style="margin-bottom:10px;">
                                <span class="tag-label">Revaçtaki Mevzuatlar:</span>
                        ...
                        <form method="POST" class="post-form" id="composer-form">
                            ...
                            <?php if ((is_user_premium($current_user_id) || is_admin()) && ($show_schedule || !empty($_POST['scheduled_at']))): ?>
    ...
    <aside class="sidebar sidebar-right">
        <div class="sidebar-section">
            ...
            <div class="sidebar-meta">
                ...
                <?php endif; ?>
                <p class="meta-row"><strong>Gönderi:</strong> <?= count($all_posts) ?> <span class="muted">· Anket: <?= $polls_count ?> · <a href="<?= BASE_PATH ?>/fotograf/<?= rawurlencode($profile_user['username']) ?>">Fotoğraf: <?= $photos_count ?></a></span></p>
                <p class="meta-row">Kuyrukta <?= $followers_count ?></p>
                <p class="meta-row">Kuyruktaki <?= $following_count ?></p>
            </div>
        </div>
        <div class="sidebar-section">
            ...
            <div class="sidebar-meta">
                <?php if (!empty($profile_user['bio'])): ?>
                    <p class="meta-row"><?= nl2br(htmlspecialchars($profile_user['bio'])) ?></p>
                <?php else: ?>
                    <p class="meta-row">Bu kullanıcı hakkında bilgi yok.</p>
                <?php endif; ?>
                <p class="meta-row">Kayıt: <?= date('d.m.Y', strtotime($profile_user['created_at'])) ?></p>
            </div>
        </div>
    </aside>
</div>
