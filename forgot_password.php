    ...
    if (isset($_POST['reset_password'])) {
        ...
        if (empty($new_password) || empty($confirm_password)) {
            ...
            if ($reset_user) {
                ...
                if (function_exists('revoke_user_url_sessions')) {
                    revoke_user_url_sessions($reset_user['id']);
                }
                $success = true;
                $step = 'email';
            } else {
        ...
        // Rate limit password reset requests (max 3 per hour per IP)
        $reset_ip = $_SERVER['REMOTE_ADDR'] ?? 'unknown';
        if (!check_rate_limit('password_reset', $reset_ip, 3, 3600)) {
            $errors[] = 'Çok fazla şifre sıfırlama isteği gönderdiniz. Lütfen bir saat sonra tekrar deneyin.';
        } elseif (empty($email_or_username)) {
