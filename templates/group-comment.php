...
<div class="posts-feed">
    ...
    <?php elseif (!$user_id): ?>
        <div class="notice notice-info text-center"><p class="muted">Yorum yapmak için <a href="<?= BASE_PATH ?>/giris" class="link-strong">giriş yapın</a>.</p></div>
    ...
    <?php else: ?>
        <div class="padded">
            <?php
            $__saved_comments = $comments;
            foreach ($comments as $comment) {
                require __DIR__ . '/group-comment-card.php';
            }
            $comments = $__saved_comments;
            unset($__saved_comments);
            ?>
        </div>
    <?php endif; ?>
</div>