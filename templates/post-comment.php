...
<?php else: ?>
    <div class="notice notice-info">
        <p class="muted">Yanıt vermek için <a href="<?= BASE_PATH ?>/login.php" class="link-strong">giriş yapın</a>.</p>
    </div>
<?php endif; ?>

<div class="posts-feed">
    <h2 class="replies-header">Yanıtlar (<?= count($replies) ?>)</h2>

    <?php if (empty($replies)): ?>
        <div class="empty-state"><p>Henüz yanıt yok. İlk yanıtı sen ver!</p></div>
    <?php else: ?>
        <?php foreach ($replies as $reply): ?>
            <?php $post = $reply; require __DIR__ . '/reply-card.php'; ?>
        <?php endforeach; ?>
    <?php endif; ?>
</div>
