...
$cm = $item['data'];
if (!empty($cm['deleted_at'])) return;
$direct_link = event_view_url((int)$cm['event_id'], $cm['event_title'] ?? '') . '#comment-' . (int)$cm['id'];
?>
<article class="post-card event-comment-feed">
    <div class="post-card-header">
        ...
        <div class="post-card-meta-row">
            <a href="<?= htmlspecialchars(event_view_url((int)$cm['event_id'], $cm['event_title'] ?? '')) ?>" class="post-card-username">📅 <?= htmlspecialchars($cm['event_title'] ?? 'Etkinlik') ?></a>
        </div>
    </div>

    <div class="post-card-content">
        <?= nl2br(linkify_text(htmlspecialchars($cm['content'] ?? '')) ) ?>
        <div class="event-comment-see-link"><a href="<?= $direct_link ?>">Etkinlikte gör</a></div>
    </div>

    <div class="post-card-actions mt-8">
        <form method="POST" action="<?= BASE_PATH ?>/api/event_comment_like.php" class="action-form action-form-inline-block">
        ...
        <?php if (!get_current_user_id()): ?>
            <a class="action-btn" href="<?= BASE_PATH ?>/giris">↩️ Yanıtla</a>
        <?php endif; ?>
        ...
        <?php if (get_current_user_id() && get_current_user_id() !== (int)($cm['user_id'] ?? 0)): ?>
            <form method="POST" action="<?= BASE_PATH ?>/api/event_comment_report.php" class="action-form action-form-report">
                <input type="hidden" name="csrf_token" value="<?= htmlspecialchars(generate_csrf_token()) ?>">
                <input type="hidden" name="comment_id" value="<?= (int)$cm['id'] ?>">
                <input type="hidden" name="event_id" value="<?= (int)$cm['event_id'] ?>">
        ...
        <?php elseif (!get_current_user_id()): ?>
            <a class="action-btn" href="<?= BASE_PATH ?>/giris">⚠️ Bildir</a>
        <?php endif; ?>
    </div>
