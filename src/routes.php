 ...
 */
// Guard in case this file is loaded without an existing router (legacy access path)
if (!isset($router) || !($router instanceof \App\Router)) {
...
$router->redirect('/poll.php', '/anket/olustur');

// ══════════════════════════════════════════════════════════════
// STATIC ROUTES — exact path → PHP file
// ══════════════════════════════════════════════════════════════
$router->map('/', 'index.php');
$router->redirect('/anasayfa', '/akis');
$router->redirect('/anasayfa/', '/akis');
$router->map('/anasayfa', 'index.php');
$router->map('/akis', 'index.php');
$router->map('/akis/kuyruk', 'index.php');
$router->map('/landing', 'landing.php');
$router->map('/landing.php', 'landing.php');
...
$router->redirect('/kurallar-sartlar/', '/kurallar-sartlar');
$router->map('/gizlilik', 'privacy.php');
$router->redirect('/gizlilik/', '/gizlilik');
$router->map('/kvkk', 'kvkk.php');
$router->map('/kvkk/', 'kvkk/index.php');
$router->map('/cerezler', 'cookie-policy.php');
$router->redirect('/cerezler/', '/cerezler');
$router->map('/javascript-kullanmiyoruz', 'nojs.php');
$router->redirect('/nojs.php', '/javascript-kullanmiyoruz');
$router->map('/guvenlik-merkezi', 'security.php');
$router->redirect('/security.php', '/guvenlik-merkezi');
$router->map('/premium', 'premium.php');
$router->map('/davet-et', 'invite.php');
$router->map('/favoriler', 'favorites.php');
$router->map('/turkce-nlp-demo', 'turkce-nlp-demo/index.php');
$router->map('/turkce-nlp-demo/', 'turkce-nlp-demo/index.php');
$router->map('/admin/bulk_optin_mail.php', 'admin/announcements.php');
$router->map('/admin/bulk-optin-mail', 'admin/announcements.php');
$router->map('/bulk-optout', 'index.php');
$router->pattern('#^/bulk-optout/([A-Fa-f0-9]{64})$#', 'index.php', [1 => 'token']);
$router->map('/sifremi-unuttum', 'forgot_password.php');
$router->map('/profil/duzenle', 'profile_edit.php');
$router->map('/dogum-gunu', 'set_birthday.php');
$router->redirect('/dogumgunu', '/dogum-gunu'); // legacy spelling, redirect to canonical
$router->map('/profil/sifre-degistir', 'password_change.php');
$router->map('/sifre', 'password_change.php');
$router->map('/anket/olustur', 'poll.php');
$router->map('/tahlil/olustur', 'test_advanced_poll.php');
$router->map('/onerilenler', 'suggestions.php');
$router->redirect('/suggestions.php', '/onerilenler');
...
// Reply compatibility: old legacy /reply.php and clean /reply/{post}/{parent}
$router->map('/reply.php', 'reply.php');
$router->pattern('#^/reply/([0-9]+)/([0-9]+)/?$#', 'reply.php', [1 => 'post_id', 2 => 'parent_id']);
$router->pattern('#^/reply/([0-9]+)/?$#', 'reply.php', [1 => 'post_id']);
...
$router->pattern('#^/post/([0-9]+)/karsilastirma/(history|gecmis|latest|son-duzenleme|[0-9]+)$#', 'post.php', [1 => 'id', 2 => 'compare']);
$router->pattern('#^/post/([0-9]+)/karsilastirma(?:/(?:latest|son-duzenleme))?$#', 'post.php', [1 => 'id', 2 => 'compare']);
$router->pattern('#^/([^/]+)/p/([0-9]+)/karsilastirma/(history|gecmis|latest|son-duzenleme|[0-9]+)$#', 'post.php', [1 => 'username', 2 => 'id', 3 => 'compare']);
$router->pattern('#^/([^/]+)/p/([0-9]+)/karsilastirma(?:/(?:latest|son-duzenleme))?$#', 'post.php', [1 => 'username', 2 => 'id', 3 => 'compare']);
$router->pattern('#^/post/([0-9]+)/edit$#', 'edit_post.php', [1 => 'id']);
$router->pattern('#^/duzenle/([0-9]+)/?$#', 'edit_post.php', [1 => 'id']);
...
$router->pattern('#^/(?:g|t)/([^/]+)/duzenle/?$#', 'group_edit.php', [1 => 'slug']);
$router->pattern('#^/(?:g|t)/([^/]+)/invite/([^/]+)/?$#', 'group_invite.php', [1 => 'slug', 2 => 'token']);
$router->pattern('#^/(?:g|t)/([^/]+)/invite/?$#', 'group_invite.php', [1 => 'slug']);
$router->pattern('#^/(?:g|t)/([^/]+)/post/([0-9]+)/karsilastirma(?:/.*)?$#', 'group_post.php', [1 => 'slug', 2 => 'id']);
$router->pattern('#^/(?:g|t)/([^/]+)/post/([0-9]+)/edit/?$#', 'group_post_edit.php', [1 => 'slug', 2 => 'id']);
$router->pattern('#^/(?:g|t)/([^/]+)/post/([0-9]+)/sil/?$#', 'group_post_delete_confirm.php', [1 => 'slug', 2 => 'id']);
...
// User-specific post compare URLs: /{username}/p/{id}/karsilastirma/... 
$router->pattern('#^/([^/]+)/p/([0-9]+)/karsilastirma/(history|gecmis|latest|son-duzenleme|[0-9]+)$#', 'post.php', [1 => 'username', 2 => 'id', 3 => 'compare']);
$router->pattern('#^/([^/]+)/p/([0-9]+)/karsilastirma(?:/(?:latest|son-duzenleme))?$#', 'post.php', [1 => 'username', 2 => 'id', 3 => 'compare']);

// User-specific post URL: /{username}/p/{id}.
$router->pattern('#^/([^/]+)/p/([0-9]+)/?$#', 'post.php', [1 => 'username', 2 => 'id']);
// Events: /etkinlik/{slug}/{id} (view); legacy /event_view.php passes through for PHP redirect
$router->pattern('#^/etkinlik/([a-zA-Z0-9_-]+)/([0-9]+)/?$#', 'event_view.php', [1 => 'slug', 2 => 'id']);
$router->pattern('#^/event_view\.php$#', 'event_view.php', []);

// Polls: /anket/olustur (create), /anket/duzenle/{id} (edit), /anket/{slug}/{id} (view)
$router->pattern('#^/anket/duzenle/([0-9]+)/?$#', 'poll.php', [1 => 'edit']);
...
// Photos: /foto/{id} (view), /fotograf-yukle (upload form), /fotograf/{username} (gallery)
// Must be registered BEFORE the generic /{username}/{id} pattern to avoid conflict.
$router->pattern('#^/foto/([0-9]+)/?$#', 'photo_view.php', [1 => 'id']);
$router->map('/fotograf-yukle', 'photo_upload.php');
$router->pattern('#^/fotograf/([^/]+)/?$#', 'photos.php', [1 => 'username']);
...
// ══════════════════════════════════════════════════════════════
// NOTE: The fallback callback runs inside a closure scope, which means
// variables set by required files (e.g. $LANG from lang.php) are NOT
// visible to functions that use "global $var". To avoid this scope issue,
// profile resolution logic sets $_GET params and requires profile.php
// after ensuring the global scope bindings work correctly.
