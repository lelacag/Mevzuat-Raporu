...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // CSRF protection.
    // When reject_cookies is set, session-stored CSRF tokens may not be available.
    // In that case, fall back to Origin/Referer header validation (same-site check).
    $reject_cookies_post = !empty($_POST['reject_cookies']) || (isset($_GET['reject_cookies']) && $_GET['reject_cookies'] == '1');
    if (!$reject_cookies_post) {
        // Normal session-based CSRF check
        if (!isset($_POST['csrf_token']) || !verify_csrf_token($_POST['csrf_token'])) {
            $errors[] = 'Invalid request. Please try again.';
        }
    } else {
        // Cookie-less flow: verify Origin or Referer is our own domain
        ...
        if (!empty($origin)) {
            ...
            $origin_ok = isset($parsed['host']) && $parsed['host'] === $allowed_host;
        } elseif (!empty($referer)) {
            $parsed = parse_url($referer);
            $origin_ok = isset($parsed['host']) && $parsed['host'] === $allowed_host;
        }
        if (!$origin_ok) {
            $errors[] = 'Invalid request origin. Please try again.';
        }
    }
    if (!empty($errors)) {
        ...
        if (empty($username) || empty($password)) {
            ...
            if ($result === true) {
                ...
                if (!empty($_SESSION['invite_token_to_accept'])) {
                    $invite_token = $_SESSION['invite_token_to_accept'];
                    unset($_SESSION['invite_token_to_accept']);
                    try {
                        $stmt = query(
                            "SELECT g.slug, gi.invite_token FROM group_invites gi JOIN groups_table g ON gi.group_id = g.id WHERE gi.invite_token = ? AND gi.status = 'pending' AND (gi.expires_at IS NULL OR gi.expires_at > NOW()) LIMIT 1",
                            [$invite_token]
                        );
                        $invite_row = $stmt->fetch(PDO::FETCH_ASSOC);
                        if ($invite_row) {
                            header('Location: ' . full_url(group_invite_url($invite_row['slug'], $invite_row['invite_token'])));
                            exit;
                        }
                    } catch (Exception $e) {
                        error_log('Invite redirect after login failed: ' . $e->getMessage());
                    }
                }
            ...
            if ($result['error'] === 'too_many_attempts') {
                $errors[] = 'Çok fazla başarısız giriş denemesi. Lütfen 15 dakika sonra tekrar deneyin.';
